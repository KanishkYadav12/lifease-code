  ...
  return (
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto px-2 pb-4 no-scrollbar">
        {isSearchMode ? (
          <div>
            <p className={`px-2 py-2 text-xs font-semibold uppercase tracking-wider ${darkMode ? "text-gray-500" : "text-gray-400"}`}>
              {searchResults.length} result{searchResults.length !== 1 ? "s" : ""}
            </p>
            {searchResults.length === 0 ? (
              <p className={`px-2 py-3 text-sm ${darkMode ? "text-gray-500" : "text-gray-400"}`}>
              ...
              searchResults.map((conv) => (
                <button
                  key={conv._id}
                  onClick={() => { onSwitchSession(conv.sessionId); clearSearch(); }}
                  className={`mb-1 w-full rounded-xl px-3 py-3 text-left transition ${
                    darkMode ? "hover:bg-gray-800" : "hover:bg-gray-50"
                  }`}
                >
                  <p className={`truncate text-sm font-medium ${darkMode ? "text-gray-200" : "text-gray-800"}`}>
                    {conv.question}
                  </p>
                  <p className={`mt-0.5 truncate text-xs ${darkMode ? "text-gray-500" : "text-gray-400"}`}>
          ...
          <p className={`px-2 py-4 text-center text-sm ${darkMode ? "text-gray-600" : "text-gray-400"}`}>
            No past chats yet.
          </p>
        ) : (
          Object.entries(grouped).map(([label, group]) =>
            group.length === 0 ? null : (
              <div key={label} className="mb-2">
                <p className={`px-2 py-1.5 text-[11px] font-semibold uppercase tracking-wider ${darkMode ? "text-gray-600" : "text-gray-400"}`}>
                  {label}
                </p>
                {group.map((s) => {
                  ...
                  return (
                    ...
                    >
                      <div className="min-w-0 flex-1">
                        ...
                        <p className={`mt-0.5 text-[11px] ${darkMode ? "text-gray-600" : "text-gray-400"}`}>
